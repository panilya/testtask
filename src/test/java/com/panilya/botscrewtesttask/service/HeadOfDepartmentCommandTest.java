...
import com.panilya.botscrewtesttask.service.commands.HeadOfDepartmentCommand;
...
public class HeadOfDepartmentCommandTest {
    ...
    void testCommandExecution() {
        ...
        mathDepartment.setHeadOfDepartment(lecturerRepository.findByName(lecturer.getName()).orElseThrow());

        departmentRepository.save(mathDepartment);

        CommandInformation command = CommandInformation.HEAD_OF_DEPARTMENT_COMMAND;
        String commandResult = headOfDepartmentCommand.executeCommand(command.getInputTemplate().formatted(mathDepartment.getName()));
        assertThat(commandResult).isEqualTo(command.getOutputTemplate().formatted(mathDepartment.getName(), lecturer.getName()));
    }

    @Test
    void testCommandExecutionWithDepartmentNameThatConsistsOfMoreThanOneWord() {
        Lecturer lecturer = LecturerObjectMother.createLecturer("Illia");
        Lecturer lecturer2 = LecturerObjectMother.createLecturer("Vlad");
        lecturerRepository.saveAll(List.of(lecturer, lecturer2));
        ...
        departmentRepository.save(DepartmentObjectMother.createDepartment("Software Engineering"));
        assertThat(departmentRepository.count()).isEqualTo(1);

        Department mathDepartment = departmentRepository.findByName("Software Engineering").orElseThrow();
        mathDepartment.addLecturer(lecturerRepository.findByName(lecturer.getName()).orElseThrow());
        mathDepartment.addLecturer(lecturerRepository.findByName(lecturer2.getName()).orElseThrow());
        mathDepartment.setHeadOfDepartment(lecturerRepository.findByName(lecturer.getName()).orElseThrow());
